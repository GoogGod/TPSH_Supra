...
def main(
    ...
    if process_data:        
        ...
        # Для прогноза нужен processed_orders.csv.
        # Если файла нет (или он пуст), подготовим его автоматически из raw Excel.
        processed_file = Path(RAW_DATA_FILE)
        raw_excel_file = Path(RAW_EXCEL_FILE)
        if (not processed_file.exists()) or processed_file.stat().st_size == 0:
            if not raw_excel_file.exists() or raw_excel_file.stat().st_size == 0:
                raise FileNotFoundError(
                    f"Нет подготовленных данных ({processed_file}) и исходного Excel ({raw_excel_file})"
                )
            if verbose:
                print("\nПодготовленные данные не найдены — запускаем preprocessing...")
            process_raw_data(
                input_file=RAW_EXCEL_FILE,
                output_file=RAW_DATA_FILE,
                verbose=verbose,
...
if __name__ == '__main__':
    waiter_config = {
        1: 'pro', 2: 'pro', 3: 'pro', 4: 'pro', 5: 'pro', 6: 'pro', 7: 'pro',
        8: 'noob', 9: 'noob'
    }
