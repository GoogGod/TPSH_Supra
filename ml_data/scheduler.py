...
WAITER_TYPES = {
    "specialist": {
        "name": "Специалист",
        "backend_role": "employee_pro",
        "capacity": 10,
        "code": 1,
    },
    "novice": {
        "name": "Новичок",
        "backend_role": "employee_noob",
        "capacity": 5,
        "code": 2,
    },
}
...
class WaiterScheduler:
    def __init__(
        ...
        if noob_ratio is not None:
            self.novice_ratio = float(noob_ratio)
        else:
            self.novice_ratio = float(novice_ratio)
        self.work_start = 10
        self.work_end = 23
        ...
        for w in active_waiters:
            ...
            for d in range(num_days):
                ...
                schedule_data.append({
                    ...
                    'waiter_num': w+1,
                    'waiter_type': WAITER_TYPES[waiter_type]['backend_role'],
                    'waiter_type_display': WAITER_TYPES[waiter_type]['name'],
